...
This module deliberately has no dependency on the Imperium/DataSnap client. It
normalizes captured TOA data and produces review decisions only. Nothing here
can close an order, transfer stock, or call an Imperium endpoint.
"""
...
def _decision(
    *,
    ...
    reasons.append("imperium_payload_validation_required")
...
def normalize_entry(entry: dict[str, Any]) -> NormalizedCapture:
    ...
    return NormalizedCapture(
        ...
        service_window=_text(activity.get("time_slot")),
        start_time=_text(activity.get("start_time")),
        end_time=_text(activity.get("end_time")),
...
def print_dry_run_report(
    ...
    print("Escrita no Imperium: DESABILITADA")
...
def _argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Valida um lote TECHCAP V5.6 sem escrever no Imperium."
    )
