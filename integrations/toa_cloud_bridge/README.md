...
4. No Dominium primário, grave a URL e a chave primária usando
   `configure_toa_cloud_bridge.py`. O arquivo resultante fica criptografado por
   usuário Windows em `config/toa_cloud_bridge_credentials.dat`.
5. No computador Central, carregue a extensão 2.6.3, abra **Detalhes > Opções da
...
- Extensão pronta: `TOA-TechNet-Bridge-2.6.3.zip`
...
npm run check
npm test
python -m unittest test_toa_connector.py
```
