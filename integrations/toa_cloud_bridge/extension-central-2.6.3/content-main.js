...
(function () {
  ...
  async function lookupContractDirect(contract, options = {}) {
    ...
    try {
      ...
      // A Time.get captured after extension startup may contain the DMV
      // buckets but not every technician. Before declaring a real TOA result
      // outside the DMV tree, read its details once: that response carries the
      // provider and parent route and completes the same in-memory tree. This
      // is read-only and preserves the route validation instead of bypassing it.
  ...
  function selectOperationalMaterials(ctx, capture) {
    if (Array.isArray(ctx?.materials) && ctx.materials.length > 0) {
      return ctx.materials;
    }
    if (Array.isArray(capture?.materials) && capture.materials.length > 0) {
      return capture.materials;
    }
    if (Array.isArray(ctx?.materialsRaw) && ctx.materialsRaw.length > 0) {
      return ctx.materialsRaw;
    }
    return Array.isArray(ctx?.materials) ? ctx.materials : (Array.isArray(capture?.materials) ? capture.materials : []);
  }
  ...
  function buildOperationalSnapshot(ctx, work) {
    ...
    const installedEquip = equipment(ctx?.installedEquipment || capture?.installed_equipment);
    const removedEquip = equipment(ctx?.removedEquipment || capture?.removed_equipment);
    const customerEquip = equipment(ctx?.customerEquipment || capture?.customer_equipment);
    const unknownEquip = equipment(ctx?.unknownEquipment || capture?.unknown_equipment);
    const opsMaterials = equipment(selectOperationalMaterials(ctx, capture));
    const capturedAuditMaterials = (Array.isArray(ctx?.materialsRaw) ? ctx.materialsRaw : []).map(item => ({
      inventory_id: String(item?.inventory_id || item?.invid || ''),
      activity_id: String(item?.activity_id || ''),
      material_code: String(item?.material_code || item?.code || ''),
    ...
    const inventoryDiagnostics = {
      inventory_count: installedEquip.length + removedEquip.length + customerEquip.length + unknownEquip.length + opsMaterials.length,
      materials_count: opsMaterials.length,
      captured_materials_count: capturedAuditMaterials.length,
  ...
  function buildDisconnectSnapshot(ctx, work) {
    ...
    return {
      ...
      inventory_diagnostics: {
        inventory_count: removed.length + added.length + unknown.length,
        materials_count: 0,
        captured_materials_count: capturedMaterials.length,
        source: 'toa-extension-direct',
      },
      responsibility: ctx?.responsibility || {},
    ...
    try {
      ...
      if ((state.searchTemplate?.entries?.length || state.syncTemplate?.entries?.length) &&
        ...
        try {
          ...
          if (direct?.error === 'toa_arvore_dmv_indisponivel') {
            state.exportStatus = `⏳ Atualizando árvore DMV — ${contrato}`;
            if (attempt.count >= LOOKUP_MAX_TRIES) {
              await ackLookup(contrato, {
                ok: false,
                reason: 'toa_route_tree_incomplete',
                source: 'toa-extension-route-tree',
                job_id: work.jobId,
              });
              clearLookupAttempt(contrato);
