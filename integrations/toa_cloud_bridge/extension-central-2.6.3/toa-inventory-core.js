(function (root, factory) {
  ...
  function normalizeActivityResponse(response, requestedAid, options = {}) {
    ...
    const relatedAids = new Set([
      aid,
      ...collectionEntries(delta.Activity).map(([key, act]) => text(act?.aid ?? key)),
    ].filter(Boolean));
    let matchedEntries = allInventory.filter(
      ([, item]) => !text(item?.inv_aid) || relatedAids.has(text(item?.inv_aid))
    );
    if (!matchedEntries.length && allInventory.length) {
      matchedEntries = allInventory;
    }
    const foreignInventoryCount = allInventory.length - matchedEntries.length;
    const inventory = matchedEntries
