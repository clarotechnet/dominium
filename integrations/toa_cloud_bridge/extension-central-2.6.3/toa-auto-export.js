(function installTOAAutoExport(root) {
  ...
  function rememberTimeResponse(payload) {
    ...
    // Depending on the OFS build/endpoint, the same provider collection is
    // returned as delta.providers, delta.Provider or Provider. Observing only
    // the first spelling left valid technicians outside the in-memory route
    // tree even though their parent DMV bucket was already known.
  ...
  root.TNTOAAutoExport = Object.freeze({
    ...
    treeStatus,
    cancelAll,
    // Read-only ingestion hook used by the silent lookup. Detail/sync
    // responses often contain a provider that was not present when the
    // extension started observing Time.get.
