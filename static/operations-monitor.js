...
(function initDominiumMonitor(root, factory) {
  ...
  function statusKind(value) {
    ...
    if (/CONCLUID|FINALIZ|BAIXAD/.test(status)) return "completed";
    if (/CANCEL/.test(status)) return "canceled";
    if (/EM CAMPO|INICIAD|EXECUT|EM ROTA/.test(status)) return "field";
    return "pending";
  }
  ...
  function routeState(value) {
    const status = normalized(value);
    if (/SUSPENS/.test(status)) return "suspended";
    if (/CONCLUID|FINALIZ|BAIXAD|ENCERRAD/.test(status)) return "completed";
    if (/CANCEL/.test(status)) return "canceled";
    if (/INICIAD|EM CAMPO|EXECUT/.test(status)) return "started";
    if (/EM ROTA/.test(status)) return "enroute";
    return "pending";
  }
  ...
  function normalizeOrder(order, selectedDate) {
    ...
    const endRaw = actualEndRaw || window.end || first(order, ["sla_end"]);
    const startRaw = actualStartRaw || window.start || first(order, ["sla_start"]);
    const orderStatus = first(order, ["toa_status", "status", "situacao", "toa_os_status"]);
    ...
    return {
      ...
      windowStartAt: parseDate(window.start, date),
      windowEndAt: parseDate(window.end, date),
      actualStartRaw: text(actualStartRaw),
      actualEndRaw: text(actualEndRaw),
      actualStartAt: parseDate(actualStartRaw, date),
      actualEndAt: parseDate(actualEndRaw, date),
  ...
  function tec1State(order, now) {
    if (!order.endAt) return { key: "unknown", label: "Sem agenda", minutes: null };
    if (order.statusKind === "completed" || order.statusKind === "canceled") {
      return { key: "done", label: "Encerrada", minutes: null };
    }
    const minutes = Math.round((order.endAt.getTime() - now.getTime()) / 60000);
    ...
    return [...groups.entries()].map(([contract, tasks]) => {
      const ordered = tasks.filter((row) => {
        ...
        return Number.isFinite(minutes) && minutes > 0 && minutes <= 30
          && !["completed", "canceled"].includes(text(row?.status_kind).toLowerCase());
      }).sort((a, b) => (
      ...
      const technicians = [...new Set(tasks
        .filter((row) => !["completed", "canceled"].includes(text(row?.status_kind).toLowerCase()))
        .map((row) => text(row.technician))
        .filter((value) => value && value !== "-"))];
      const activeTasks = tasks.filter(
        (row) => !["completed", "canceled"].includes(text(row?.status_kind).toLowerCase()),
      );
  ...
  function orderRow(order, now) {
    ...
    return {
      ...
      schedule: order.startRaw || order.endRaw ? `${order.startRaw || "-"} - ${order.endRaw || "-"}` : "Sem agenda",
      tec1: tec1.label,
      tec1_kind: tec1.key,
      tec1_minutes: tec1.minutes,
      tec1_deadline: order.endAt ? order.endAt.toISOString() : "",
  ...
  function routeAlert(order, now) {
    if (order.isAuxiliary || !order.windowEndAt || ["completed", "canceled", "suspended"].includes(order.routeState)) return null;
  ...
  function buildRouteConsole(orders, now) {
    ...
    activityGroups.forEach((activityGroup) => {
      ...
      const group = groups.get(technician) || {
        technician,
        technician_login: order.technicianLogin || "-",
        bucket: order.bucket || "-",
        total: 0,
      ...
      const timelineStart = order.actualStartRaw || order.windowStartRaw || order.startRaw;
      const timelineEnd = order.actualEndRaw || order.windowEndRaw || order.endRaw;
  ...
  function buildTvDashboard(model) {
    ...
    const activeRows = rows.filter((row) => !["completed", "canceled"].includes(row.status_kind));
    const tec1Rank = { late: 0, risk: 1, safe: 2, unknown: 3, done: 4 };
    const tec1Rows = activeRows.slice().sort((a, b) => (
    ...
    const activeTechnicians = (routeConsole.technicians || []).map((technician) => {
      const current = (technician.activities || []).find((activity) => activity.route_state === "started")
        || (technician.activities || []).find((activity) => activity.route_state === "enroute");
      return current ? {
    ...
    return {
      ...
      kpis: { ...(model?.kpis || {}), tec1Risk: urgentTec1.length, tec1Late: urgentTec1.filter((row) => row.tec1_kind === "late").length },
      tec1Rows: (urgentTec1.length ? urgentTec1 : tec1Rows).slice(0, 12),
      activeTechnicians,
